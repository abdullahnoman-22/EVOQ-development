...
// Global checkImagesLoaded function for image comparison sections
...
// Global toggleReadMore function for product feature sections
...
// Global changeProduct function for product carousel
function changeProduct(direction) {
  ...
  slides.forEach((slide, index) => {
    if (slide.classList.contains('cus-pf-active')) {
...
// Mobile button fix and error handling
document.addEventListener('DOMContentLoaded', function() {
  // Fix for mobile buttons that aren't working
  const allButtons = document.querySelectorAll('button, a[class*="button"], a[class*="cus-"], .button, .cus-e-shop-all-btn, .cus-pro-cta-button, .cus-h-cta-button-2');
  ...
  allButtons.forEach(button => {
    ...
    // Add click event if href exists but button seems unresponsive
    if (button.tagName === 'A' && button.href && !button.onclick) {
      ...
      // Also add touchend for better mobile response
      button.addEventListener('touchend', function(e) {
        if (!e.defaultPrevented) {
          try {
            window.location.href = this.href;
          } catch (error) {
            console.log('Touch navigation error handled');
          }
        }
      });
    }
...
window.onerror = function(msg, url, lineNo, columnNo, error) {
  console.log('JavaScript Error handled:', msg);
  return true; // Prevents default browser error handling
};

window.addEventListener('error', function(event) {
  console.log('Error handled:', event.error);
  return true;
});
